...
When `main` is not checked out anywhere, the helper uses a guarded
`git update-ref` with the observed local SHA as the expected old value.
...
local `main` ahead/behind distance, fast-forwards the local `main` ref only when
it is a safe ancestor of `origin/main` and `behind>0` was observed, and then
reports `dirty_count`, `repo_main_behind`, `repo_main_aligned`, and the derived
`request_action`. For post-repair inspection without fetching or mutating any
refs, run:
